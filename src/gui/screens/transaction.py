...
class TransactionScreen(Prompt):
    def __init__(self, title, meta):
        ...
        self.details_sw.add_event_cb(lambda event: self.toggle_details(), lv.EVENT.VALUE_CHANGED, None)
        if enable_inputs:
        ...
        style.set_text_font(lv.font_roboto_mono_28)

        style_primary = lv.style_t()
        style_primary.init()
        style_primary.set_text_color(lv.color_hex(0xFFFFFF))
        style_primary.set_text_font(lv.font_roboto_mono_22)
        ...
        style_gray.init()
        style_gray.set_text_color(lv.color_hex(0x999999))
        style_gray.set_text_font(lv.font_roboto_mono_22)
        ...
        for i, inp in enumerate(meta["inputs"]):
            ...
            if inp.get("sighash", ""):
                ...
                shlbl.set_x(60)
                shlbl.add_style(style_warning, 0)
                lbl = shlbl
            obj = lbl

        meta_outputs_len = len(meta["outputs"])
        lbl = add_label("%d %s" % (meta_outputs_len, "OUTPUT" if meta_outputs_len == 1 else "OUTPUTS"), scr=self.page2)
        lbl.add_style(style, 0)
        ...
        for i, out in enumerate(meta["outputs"]):
            ...
            addrlbl.set_x(60)
            if out.get("label", ""):
                addrlbl.add_style(style_secondary, 0)
            else:
        ...
        if all(inp["sequence"] == 0xFFFFFFFF for inp in meta["inputs"]):
            ...
            ltdatelbl.add_style(style_gray, 0)
    ...
    def show_output(self, out, obj):
        ...
        if out.get("label", ""):
            addr.add_style(self.style_secondary, 0)
        else:
