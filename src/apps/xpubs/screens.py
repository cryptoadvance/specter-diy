...
class XPubScreen(QRAlert):
    ...
    def __init__(
        ...
        if prefix is not None:
            self.prefix_switch.add_event_cb(lambda event: self.toggle_event(), lv.EVENT.VALUE_CHANGED, None)
        if slip132 is not None:
            self.slip_switch.add_event_cb(lambda event: self.toggle_event(), lv.EVENT.VALUE_CHANGED, None)
        add_button_pair(
