...
class WalletScreen(QRAlert):
    def __init__(self, wallet, network, idx=None, branch_index=0):
        ...
        self.title.add_event_cb(on_release(self.rename), lv.EVENT.CLICKED, None)
        self.policy = add_label(wallet.policy, y=55, style="hint", scr=self)

        self.message_style = lv.style_t()
        self.message_style.init()
        self.message_style.set_text_color(lv.color_hex(0xFFFFFF))
        self.message_style.set_text_font(lv.font_roboto_mono_22)
        self.message.add_style(self.message_style, 0)
        ...
        self.warning_style = lv.style_t()
        self.warning_style.init()
        self.warning_style.set_text_color(lv.color_hex(0xFF9A00))
        self.warning.add_style(self.warning_style, 0)
...
def _build_screen(scr, policy, keys):
    ...
    if need_slip132_switch:
        ...
        scr.slip_switch.align_to(lbl, lv.ALIGN.CENTER, 0, 0)
        scr.slip_switch.add_event_cb(lambda event: scr.fill_message(), lv.EVENT.VALUE_CHANGED, None)
    else:
